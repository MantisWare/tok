# TOK Token Savings Audit Guide

Complete guide to analyzing your tok token savings with temporal breakdowns and data exports.

## Overview

The `tok gain` command provides comprehensive analytics for tracking your token savings across time periods.

**Database Location**: `~/.local/share/tok/history.db`
...
# Temporal breakdowns
tok gain --daily          # All days since tracking started
tok gain --weekly         # Aggregated by week
tok gain --monthly        # Aggregated by month
tok gain --all            # Show all breakdowns at once

# Export formats
tok gain --all --format json > savings.json
tok gain --all --format csv > savings.csv

# Combined flags
tok gain --graph --history --quota    # Classic view with extras
tok gain --daily --weekly --monthly   # Multiple breakdowns
```
...
**Metrics explained:**
- **Cmds**: Number of tok commands executed
- **Input**: Estimated tokens from raw command output
- **Output**: Actual tokens after tok filtering
...
tok gain --weekly --format csv > reports/week-$(date +%Y-%W).csv

# Compare this week vs last week
tok gain --weekly | tail -3
```
...
tok gain --monthly --format json | jq '.monthly[] |
...
# Get CSV data
result = subprocess.run(['tok', 'gain', '--all', '--format', 'csv'],
...
1. Export CSV: `tok gain --all --format csv > tok-data.csv`
2. Open in Excel
...
0 0 * * * tok gain --all --format json > /var/www/dashboard/tok-stats.json
...
tok estimates tokens using `text.len() / 4` (4 characters per token average).
...
Output Tokens   = estimate_tokens(tok_filtered_output)
...
|---------|----------------|-----------|
| `tok git status` | 77-93% | Compact stat format |
| `tok eslint` | 84% | Group by rule |
| `tok vitest run` | 94-99% | Show failures only |
| `tok find` | 75% | Tree format |
| `tok pnpm list` | 70-90% | Compact dependencies |
| `tok grep` | 70% | Truncate + group |
...
```bash
# Location
ls -lh ~/.local/share/tok/history.db

# Schema
sqlite3 ~/.local/share/tok/history.db ".schema"

# Recent records
sqlite3 ~/.local/share/tok/history.db \
  "SELECT timestamp, tok_cmd, saved_tokens FROM commands
...
```bash
# Backup
cp ~/.local/share/tok/history.db ~/backups/tok-history-$(date +%Y%m%d).db

# Restore
cp ~/backups/tok-history-20260128.db ~/.local/share/tok/history.db

# Export for migration
sqlite3 ~/.local/share/tok/history.db .dump > tok-backup.sql
```
...
# Reset all data
rm ~/.local/share/tok/history.db
# Next tok command will recreate database
```
...
```yaml
# .github/workflows/tok-stats.yml
name: TOK Stats Report
on:
  schedule:
  ...
  stats:
    ...
    steps:
      ...
      - name: Generate report
        run: |
          tok gain --weekly --format json > stats/week-$(date +%Y-%W).json
      - name: Commit stats
        run: |
          git add stats/
          git commit -m "Weekly tok stats"
          git push
```
...
def send_tok_stats():
    result = subprocess.run(['tok', 'gain', '--format', 'json'],
                           capture_output=True, text=True)
    data = json.loads(result.stdout)

    message = f"""
    📊 *TOK Token Savings Report*
...
# Check record count
sqlite3 ~/.local/share/tok/history.db "SELECT COUNT(*) FROM commands"
...
tok gain --format json 2>&1 | tee /tmp/tok-debug.log | jq .

# Use release build to avoid warnings
cargo build --release
./target/release/tok gain --format json
```
...
# Validate estimation
tok git status > output.txt
python -c "
import tiktoken
enc = tiktoken.get_encoding('cl100k_base')
text = open('output.txt').read()
print(f'Actual tokens: {len(enc.encode(text))}')
print(f'tok estimate: {len(text) // 4}')
"
```

## Best Practices

1. **Regular Exports**: `tok gain --all --format json > monthly-$(date +%Y%m).json`
...
- [README.md](../README.md) - Full tok documentation
