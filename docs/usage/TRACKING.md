# TOK Tracking API Documentation

Comprehensive documentation for TOK's token savings tracking system.
...
TOK's tracking system records every command execution to provide analytics on token savings. The system:
- Stores command history in SQLite (~/.local/share/tok/tracking.db)
...
```
tok command execution
  ↓
TimedExecution::start()
  ↓
[command runs]
  ↓
TimedExecution::track(original_cmd, tok_cmd, input, output)
  ↓
Tracker::record(original_cmd, tok_cmd, input_tokens, output_tokens, exec_time_ms)
  ↓
SQLite database (~/.local/share/tok/tracking.db)
  ↓
Aggregation APIs (get_summary, get_all_days, etc.)
  ↓
CLI output (tok gain) or JSON/CSV export
```

### Storage Location

- **Linux**: `~/.local/share/tok/tracking.db`
- **macOS**: `~/Library/Application Support/tok/tracking.db`
- **Windows**: `%APPDATA%\tok\tracking.db`
...
impl Tracker {
    ...
    pub fn record(
        ...
        tok_cmd: &str,            // TOK command (e.g., "tok ls")
...
pub struct CommandRecord {
    ...
    pub tok_cmd: String,           // TOK command used
...
pub fn track(original_cmd: &str, tok_cmd: &str, input: &str, output: &str);
```
...
fn main() -> anyhow::Result<()> {
    ...
    timer.track_passthrough("git tag --list", "tok git tag --list");
...
```yaml
# .github/workflows/track-tok-savings.yml
name: Track TOK Savings
  ...
  track-savings:
    ...
    steps:
      ...
      - name: Export weekly stats
        run: |
          tok gain --weekly --format json > tok-weekly.json
          cat tok-weekly.json
      ...
      - name: Post to Slack
        ...
        run: |
          SAVINGS=$(jq -r '.[0].saved_tokens' tok-weekly.json)
          PCT=$(jq -r '.[0].savings_pct' tok-weekly.json)
          curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"📊 TOK Weekly: ${SAVINGS} tokens saved (${PCT}%)\"}" \
            $SLACK_WEBHOOK
```
...
#!/usr/bin/env python3
"""
Export TOK metrics to Grafana/Datadog/etc.
"""
import json
import subprocess
...
def get_tok_metrics():
    """Fetch TOK metrics as JSON."""
    result = subprocess.run(
        ["tok", "gain", "--all", "--format", "json"],
        capture_output=True,
        text=True
    )
...
CREATE TABLE commands (
    ...
    tok_cmd TEXT NOT NULL,             -- TOK command (e.g., "tok ls")
...
- **Telemetry enabled by default**: TOK sends a daily anonymous usage ping (version, OS, command counts, token savings). Device identity is a salted SHA-256 hash. Opt out with `TOK_TELEMETRY_DISABLED=1` or `[telemetry] enabled = false` in `~/.config/tok/config.toml`
- **User control**: Users can delete `~/.local/share/tok/tracking.db` anytime
...
- Ensure only one TOK process writes at a time
- Check file permissions on `~/.local/share/tok/tracking.db`
- Delete and recreate: `rm ~/.local/share/tok/tracking.db && tok gain`

### Missing exec_time_ms column

Older databases may not have the `exec_time_ms` column. TOK automatically migrates on first use, but you can force it:

```bash
sqlite3 ~/.local/share/tok/tracking.db \
  "ALTER TABLE commands ADD COLUMN exec_time_ms INTEGER DEFAULT 0"
```
...
- [COMMAND_AUDIT.md](../claudedocs/COMMAND_AUDIT.md) - List of all TOK commands
