...
> **Deep reference** for TOK's system design, filtering taxonomy, performance characteristics, and architecture decisions. For a guided tour of the end-to-end flow, start with [TECHNICAL.md](TECHNICAL.md).

**tok (Token Optimization Kit)** is a high-performance CLI proxy that minimizes LLM token consumption through intelligent output filtering and compression.
...
> For the proxy pattern diagram and key components table, see [TECHNICAL.md](TECHNICAL.md#2-architecture-overview). For day-to-day build and test commands, see [DEVELOPMENT.md](DEVELOPMENT.md).
...
Phase 2: ROUTE
──────────────
main.rs: cli_dispatch::dispatch(cli)
  ↓
cmds/git/git.rs (example): git::run(args, verbose)
...
Database: `history.db` under the platform tok data dir (see **Token Tracking System** below).
```
...
**Scale (approximate)**: on the order of **50+** Rust sources under `src/cmds/` (per-ecosystem `mod.rs` plus command implementations), plus shared crates in `core/`, `hooks/`, `discover/`, `analytics/`, etc. Exact counts drift as filters are added — use the [folder map in TECHNICAL.md](TECHNICAL.md#4-folder-map) as the source of structure.

### Module Breakdown

- **CLI surface**: `main.rs` defines `Cli`, global flags (`-v`, `-u`, `--skip-env`), and the full `Commands` graph; `run_cli()` performs telemetry, parse, hook warning, integrity gate, then calls `cli_dispatch::dispatch()`.
- **Dispatch**: [`src/cli_dispatch.rs`](../src/cli_dispatch.rs) — single `match` from `Commands` into ecosystem modules.
- **Command modules**: `src/cmds/` — by ecosystem (git, rust, js, python, go, dotnet, cloud, system, ruby). Each ecosystem README lists its files.
- **Core infrastructure**: `src/core/` — config, constants, `display_helpers`, `filter`, `runner`, `tee`, `toml_filter`, `tracking`, `telemetry`, `utils`.
...
- **Git**: status, diff, log, add, commit, push, branch/checkout, plus `gh` / `gt` integrations (see `cmds/git/README.md`).
- **JS/TS**: npm, pnpm, lint, tsc, next, prettier, playwright, prisma, vitest (see `cmds/js/README.md`).
- **Python**: ruff, pytest, mypy, pip (see `cmds/python/README.md`).
- **Go**: `go` subcommands and golangci-lint (see `cmds/go/README.md`).
...
- Natural CLI: `tok go test` not `tok gotest`
...
Command                 Raw Time    tok Time    Overhead    Savings
─────────────────────────────────────────────────────────────────────────
...
4. STORAGE
   ───────
   Database file: `history.db` inside the tok data directory (e.g. `~/.local/share/tok/` on Linux, `~/Library/Application Support/tok/` on macOS; see `TOK_DATA_DIR` / `HISTORY_DB` in `src/core/constants.rs`).
...
main.rs (Cli): `ultra_compact`
#[arg(short = 'u', long, global = true)]
ultra_compact: bool,

**`--skip-env`**: When set, TOK propagates `SKIP_ENV_VALIDATION=1` to selected child processes (see `Cli` in `main.rs`) so tools like Next.js / tsc / Prisma skip strict env checks in agent workflows.
...
`main()` → `run_cli()` → `Result<i32>`
  ↓
`cli_dispatch::dispatch(cli)` matches `cli.command` and calls the relevant `cmds/**` handler (each returns `Result<i32>` or nested `Result` that maps to an exit code).
  ↓ `.context("…")?` at subsystem boundaries
Child `std::process::Command` or filter logic
  ↓ Error occurs
`anyhow::Error`
  ↓ Bubble up through `?`
`main()` maps `Err` to:
  ↓
`eprintln!("tok: {:#}", e)` then `std::process::exit(1)`  
`Ok(code)` → `std::process::exit(code)` (preserves child exit codes)
```
...
├─────────┼──────────────────────────────────────────────────────┤
│ 0       │ Success                                              │
│ 1       │ tok internal error (parsing, filtering, etc.)        │
...
Two tiers: **User settings** (`~/.config/tok/config.toml`) and **LLM integration** (CLAUDE.md via `tok init`).
...
Check existing CLAUDE.md:
  • --global? → ~/.config/tok/CLAUDE.md
  • else      → ./CLAUDE.md
      ↓
      ├─ Exists? → Warn user, ask to overwrite
      └─ Not exists? → Continue
      ↓
Prompt: "Initialize tok for LLM usage? [y/N]"
      ↓ Yes
Write template:
┌─────────────────────────────────────┐
│ # CLAUDE.md                         │
│                                     │
│ Use `tok` prefix for commands:      │
│ - tok git status                    │
│ - tok lint                          │
│ - tok test                          │
│                                     │
...
└─────────────────────────────────────┘
      ↓
Success: "✓ Initialized tok for LLM integration"
```
...
┌──────────────────────┬──────────────┬──────────────┐
│ Operation            │ tok Overhead │ Total Time   │
├──────────────────────┼──────────────┼──────────────┤
│ tok git status       │ +8ms         │ 58ms         │
│ tok grep "pattern"   │ +12ms        │ 145ms        │
│ tok read file.rs     │ +5ms         │ 15ms         │
│ tok lint             │ +15ms        │ 2.5s         │
└──────────────────────┴──────────────┴──────────────┘
...
> For the complete step-by-step process to add a new command (module file, `Commands` variant in `main.rs`, `cli_dispatch` arm, `discover/rules` when rewriting, tests), see [src/cmds/README.md — Adding a New Command Filter](../src/cmds/README.md#adding-a-new-command-filter).

New **operational** commands that run through the hook pipeline must also be added to `is_operational_command()` in `main.rs` if they should participate in the runtime integrity check.
...
- **Global Flags**: `-v`, `-u`, and `--skip-env` apply across subcommands (see `Cli` in `main.rs`)
...
- **[DEVELOPMENT.md](DEVELOPMENT.md)**: Build, test, smoke scripts, release notes for maintainers
...
| **Proxy Pattern** | tok sits between user and tool, transforming output |
...
| **Skip-env** | Global `--skip-env` sets `SKIP_ENV_VALIDATION=1` for compatible child tools |
