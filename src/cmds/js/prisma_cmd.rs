...
fn run_generate(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.track("prisma generate", "tok prisma generate", &raw, &raw);
        return Ok(exit_code);
    }

    let filtered = filter_prisma_generate(&raw);
    println!("{}", filtered);
    timer.track("prisma generate", "tok prisma generate", &raw, &filtered);
...
fn run_migrate(subcommand: MigrateSubcommand, args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.track(cmd_name, &format!("tok {}", cmd_name), &raw, &raw);
        return Ok(exit_code);
    }
    ...
    println!("{}", filtered);
    timer.track(cmd_name, &format!("tok {}", cmd_name), &raw, &filtered);
...
fn run_db_push(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.track("prisma db push", "tok prisma db push", &raw, &raw);
        return Ok(exit_code);
    }

    let filtered = filter_db_push(&raw);
    println!("{}", filtered);
    timer.track("prisma db push", "tok prisma db push", &raw, &filtered);
