...
pub fn run_err(command: &str, verbose: u8) -> Result<i32> {
    ...
    if filtered.is_empty() {
        if output.status.success() {
            tok.push_str("[ok] Command completed successfully (no errors)");
        } else {
            tok.push_str(&format!(
    ...
    timer.track(command, "tok run-err", &raw, &tok);
    Ok(exit_code)
}
...
pub fn run_test(command: &str, verbose: u8) -> Result<i32> {
    ...
    timer.track(command, "tok run-test", &raw, &summary);
    Ok(exit_code)
}
