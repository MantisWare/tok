...
pub fn run(file1: &Path, file2: &Path, verbose: u8) -> Result<()> {
    ...
    if diff.added == 0 && diff.removed == 0 {
        tok.push_str("[ok] Files are identical");
        println!("{}", tok);
        timer.track(
    ...
    for change in &diff.changes {
        match change {
            DiffChange::Added(ln, c) => tok.push_str(&format!("+{:4} {}\n", ln, c)),
            DiffChange::Removed(ln, c) => tok.push_str(&format!("-{:4} {}\n", ln, c)),
            DiffChange::Modified(ln, old, new) => {
                tok.push_str(&format!("~{:4} {} → {}\n", ln, old, new))
            }
        }
    }
...
pub fn run_stdin(_verbose: u8) -> Result<()> {
    ...
    timer.track("diff (stdin)", "tok diff (stdin)", &input, &condensed);
