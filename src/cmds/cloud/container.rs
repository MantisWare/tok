    ...
    F: Fn(&Value) -> String,
{
    runner::run_filtered(
        ...
        |stdout| match serde_json::from_str::<Value>(stdout) {
            ...
            Err(e) => {
                eprintln!("[tok] kubectl: JSON parse failed: {}", e);
                stdout.to_string()
            }
        },
...
fn docker_ps(_verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        eprint!("{}", stderr);
        timer.track("docker ps", "tok docker ps", &raw, &raw);
    ...
    if stdout.trim().is_empty() {
        tok.push_str("[docker] 0 containers");
        println!("{}", tok);
        timer.track("docker ps", "tok docker ps", &raw, &tok);
        return Ok(0);
    }

    let count = stdout.lines().count();
    tok.push_str(&format!("[docker] {} containers:\n", count));
    ...
    if count > 15 {
        tok.push_str(&format!("  ... +{} more", count - 15));
    }
...
fn docker_images(_verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        eprint!("{}", stderr);
        timer.track("docker images", "tok docker images", &raw, &raw);
        return Ok(exit_code_from_output(&output, "docker"));
    }
    ...
    if lines.is_empty() {
        tok.push_str("[docker] 0 images");
        println!("{}", tok);
        timer.track("docker images", "tok docker images", &raw, &tok);
        return Ok(0);
    }
    ...
    if lines.len() > 15 {
        tok.push_str(&format!("  ... +{} more", lines.len() - 15));
    }
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<i32> {
    ...
    if container.is_empty() {
        println!("Usage: tok docker logs <container>");
        return Ok(0);
    }
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<i32> {
    ...
    if pod.is_empty() {
        println!("Usage: tok kubectl logs <pod>");
        return Ok(0);
    }
...
pub fn run_compose_ps(verbose: u8) -> Result<i32> {
    ...
    let tok = format_compose_ps(&structured);
    println!("{}", tok);
    timer.track("docker compose ps", "tok docker compose ps", &raw, &tok);
    Ok(0)
}
