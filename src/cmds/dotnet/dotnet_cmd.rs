...
pub fn run_format(args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track(
        ...
        &format!("tok dotnet format {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
fn run_dotnet_with_binlog(subcommand: &str, args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track(
        ...
        &format!("tok dotnet {} {}", subcommand, args.join(" ")),
        &raw,
        &output_to_print,
    );
...
fn build_trx_results_dir() -> PathBuf {
    std::env::temp_dir().join(format!("tok_dotnet_testresults_{}", unique_temp_suffix()))
}
...
fn build_format_report_path() -> PathBuf {
    std::env::temp_dir().join(format!("tok_dotnet_format_{}.json", unique_temp_suffix()))
}
