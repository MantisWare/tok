...
/// Native find flags that TOK cannot handle correctly.
...
/// Parse arguments from raw args vec, supporting both native find and TOK syntax.
///
/// Native find syntax: `find . -name "*.rs" -type f -maxdepth 3`
/// TOK syntax: `find *.rs [path] [-m max] [-t type]`
fn parse_find_args(args: &[String]) -> Result<FindArgs> {
    ...
    if has_unsupported_find_flags(args) {
        anyhow::bail!(
            "tok find does not support compound predicates or actions (e.g. -not, -exec). Use `find` directly."
        );
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            flag if flag.starts_with('-') => {
                eprintln!("tok find: unknown flag '{}', ignored", flag);
            }
            _ => {}
        }
        i += 1;
...
/// Parse TOK syntax: `find <pattern> [path] [-m max] [-t type]`
fn parse_tok_find_args(args: &[String]) -> Result<FindArgs> {
...
pub fn run(
    ...
    let tok_output = format!("{}F {}D + {}", total_files, dirs_count, ext_line);
    timer.track(
        &format!("find {} -name '{}'", path, effective_pattern),
        "tok find",
        &raw_output,
...
mod tests {
    ...
    // --- parse_find_args: TOK syntax ---

    #[test]
    fn parse_tok_syntax_pattern_only() {
    ...
    #[test]
    fn parse_tok_syntax_pattern_and_path() {
    ...
    #[test]
    fn parse_tok_syntax_with_flags() {
    ...
    #[test]
    fn run_from_args_tok_syntax() {
        // Simulates: tok find *.rs src
