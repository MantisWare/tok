...
pub fn run(
    ...
    if filtered.trim().is_empty() && !content.trim().is_empty() {
        eprintln!(
            "tok: warning: filter produced empty output for {} ({} bytes), showing raw content",
            file.display(),
            content.len()
        );
    ...
    let tok_output = if line_numbers {
...
pub fn run_stdin(
    ...
    timer.track("cat - (stdin)", "tok read -", &content, &tok_output);
    Ok(())
}
...
fn main() {{
    ...
    fn test_read_valid_and_nonexistent() {
        ...
        let output = std::process::Command::new(&bin)
            .args(["read", &f1.path().to_string_lossy(), "/tmp/tok_nonexistent_file.txt"])
            .output()
            .expect("failed to run tok read");
        ...
        assert!(stderr.contains("tok_nonexistent_file"), "should report missing file on stderr");
    }
