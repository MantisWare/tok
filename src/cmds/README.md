...
6. **Exit code** — returns `Ok(exit_code)` to caller; `main.rs` exits the process with that code once `cli_dispatch` returns
...
| `Err(e)` | TOK itself failed (not the command) | `main.rs` prints error, exits 1 |
