...
const TELEMETRY_URL: Option<&str> = option_env!("TOK_TELEMETRY_URL");
const TELEMETRY_TOKEN: Option<&str> = option_env!("TOK_TELEMETRY_TOKEN");
...
pub fn maybe_ping() {
    ...
    if std::env::var("TOK_TELEMETRY_DISABLED").unwrap_or_default() == "1" {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn test_salt_file_path_is_in_tok_dir() {
        let path = salt_file_path();
        assert!(path.to_string_lossy().contains("tok"));
    ...
    fn test_install_method_unix_paths() {
        assert_eq!(
            install_method_from_path("/opt/homebrew/Cellar/tok/0.28.0/bin/tok"),
            "homebrew"
        );
        assert_eq!(
            install_method_from_path("/usr/local/homebrew/bin/tok"),
            "homebrew"
        );
        assert_eq!(
        ...
        assert_eq!(install_method_from_path("/usr/bin/tok"), "other");
    }

    #[test]
    fn test_install_method_windows_paths() {
        assert_eq!(
            install_method_from_path("C:\\Users\\user\\.cargo\\bin\\tok.exe"),
            "cargo"
        );
        assert_eq!(
            install_method_from_path("C:\\Users\\user\\.local\\bin\\tok.exe"),
            "script"
        );
        assert_eq!(
            install_method_from_path("C:\\Program Files\\tok\\tok.exe"),
            "other"
        );
    }
