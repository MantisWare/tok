...
use super::constants::{CONFIG_TOML, DEFAULT_HISTORY_DAYS, TOK_DATA_DIR};
use anyhow::Result;
...
pub struct HooksConfig {
    ...
    /// Survives `tok init -g` re-runs since config.toml is user-owned.
    #[serde(default)]
...
fn get_config_path() -> Result<PathBuf> {
    ...
    Ok(config_dir.join(TOK_DATA_DIR).join(CONFIG_TOML))
}
