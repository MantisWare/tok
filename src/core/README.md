...
## Purpose
Core infrastructure shared by all TOK command modules. Every filter, tracker, and command handler depends on these modules. No inward dependencies — leaf in the dependency graph (no circular imports possible).
...
1. `.tok/filters.toml` (project-local, requires `tok trust`)
2. `~/.config/tok/filters.toml` (user-global)
