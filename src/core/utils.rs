...
//!
//! Provides common helpers used across tok commands:
...
/// # Examples
/// ```
/// use tok::utils::join_with_overflow;
...
/// # Examples
/// ```
/// use tok::utils::truncate_iso_date;
        ...
        None => {
            #[cfg(unix)]
            {
                ...
                if let Some(sig) = output.status.signal() {
                    eprintln!("[tok] {}: process terminated by signal {}", label, sig);
                    return 128 + sig;
                }
            }
            eprintln!("[tok] {}: process terminated by signal", label);
            1
        }
    }
}
...
pub fn fallback_tail(output: &str, label: &str, n: usize) -> String {
    eprintln!(
        "[tok] {}: output format not recognized, showing last {} lines",
        label, n
    );
...
/// # Examples
/// ```no_run
/// use tok::utils::detect_package_manager;
...
pub fn resolved_command(name: &str) -> Command {
    match resolve_binary(name) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "tok: Failed to resolve '{}' via PATH, falling back to direct exec: {}",
                name, e
            );
...
mod tests {
    ...
    fn test_detect_package_manager_default() {
        // In the test environment (tok repo), there's no JS lockfile
