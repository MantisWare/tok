...
///   1. `.tok/filters.toml`              — project-local, committable with the repo
///   2. `~/.config/tok/filters.toml`     — user-global, applies to all projects
...
///
/// `tok init` generates a commented template for both levels (project or global).
///
/// Environment variables:
///   - `TOK_NO_TOML=1`     — bypass TOML engine entirely
///   - `TOK_TOML_DEBUG=1`  — print which filter matched and line counts to stderr
///
...
use super::constants::{FILTERS_TOML, TOK_DATA_DIR};
...
impl TomlFilterRegistry {
    ...
    fn load() -> Self {
        ...
        // Priority 1: project-local .tok/filters.toml (trust-gated)
        let project_filter_path = std::path::Path::new(".tok/filters.toml");
                ...
                | crate::hooks::trust::TrustStatus::EnvOverride => {
                    if let Ok(content) = std::fs::read_to_string(project_filter_path) {
                        match Self::parse_and_compile(&content, "project") {
                            ...
                            Err(e) => eprintln!("[tok] warning: .tok/filters.toml: {}", e),
                        }
                    }
                }
                crate::hooks::trust::TrustStatus::Untrusted => {
                    eprintln!("[tok] WARNING: untrusted project filters (.tok/filters.toml)");
                    eprintln!("[tok] Filters NOT applied. Run `tok trust` to review and enable.");
                }
                crate::hooks::trust::TrustStatus::ContentChanged { .. } => {
                    eprintln!("[tok] WARNING: .tok/filters.toml changed since trusted.");
                    eprintln!("[tok] Filters NOT applied. Run `tok trust` to re-review.");
                }
            }
        }

        // Priority 2: user-global ~/.config/tok/filters.toml
        if let Some(config_dir) = dirs::config_dir() {
            let global_path = config_dir.join(TOK_DATA_DIR).join(FILTERS_TOML);
            if let Ok(content) = std::fs::read_to_string(&global_path) {
                match Self::parse_and_compile(&content, "user-global") {
                    Ok(f) => filters.extend(f),
                    Err(e) => eprintln!("[tok] warning: {}: {}", global_path.display(), e),
                }
            }
        }
        ...
        match Self::parse_and_compile(builtin, "builtin") {
            Ok(f) => filters.extend(f),
            Err(e) => eprintln!("[tok] warning: builtin filters: {}", e),
        }
    ...
    fn parse_and_compile(content: &str, source: &str) -> Result<Vec<CompiledFilter>, String> {
        ...
        for (name, def) in file.filters {
            match compile_filter(name.clone(), def) {
                ...
                Err(e) => eprintln!("[tok] warning: filter '{}' in {}: {}", name, source, e),
            }
        }
        Ok(compiled)
    }
...
fn compile_filter(name: String, def: TomlFilterDef) -> Result<CompiledFilter, String> {
    ...
    for cmd in RUST_HANDLED_COMMANDS {
        if match_regex.is_match(cmd) {
            eprintln!(
                "[tok] warning: filter '{}' match_command matches '{}' which is already \
...
// ---------------------------------------------------------------------------
// tok verify — inline test execution
// ---------------------------------------------------------------------------
...
pub fn run_filter_tests(filter_name_opt: Option<&str>) -> VerifyResults {
    ...
    // Trust-gated: only verify project-local filters if trusted (SA-2025-TOK-002)
    let project_path = std::path::Path::new(".tok/filters.toml");
    if project_path.exists() {
        ...
        match trust_status {
            ...
            _ => {
                eprintln!("[tok] WARNING: untrusted project filters skipped in verify");
            }
        }
    }
...
fn collect_test_outcomes(
    ...
    let file: TomlFilterFile = match toml::from_str(content) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("[tok] warning: TOML parse error during verify: {}", e);
            return;
        }
    };
    ...
    for (name, def) in file.filters {
        ...
        match compile_filter(name.clone(), def) {
            ...
            Err(e) => eprintln!("[tok] warning: filter '{}' compilation error: {}", name, e),
        }
    }
    ...
    for (filter_name, tests) in file.tests {
        ...
        let compiled = match compiled_filters.get(&filter_name) {
            ...
            None => {
                eprintln!(
                    "[tok] warning: [[tests.{}]] references unknown filter",
                    filter_name
                );
                continue;
            }
...
pub fn find_matching_filter(command: &str) -> Option<&'static CompiledFilter> {
    if std::env::var("TOK_TOML_DEBUG").is_ok() {
        eprintln!(
            "[tok:toml] looking up filter for: {:?} ({} filters loaded)",
            command,
    ...
    if std::env::var("TOK_TOML_DEBUG").is_ok() {
        match result {
            Some(f) => eprintln!("[tok:toml] matched filter: '{}'", f.name),
            None => eprintln!("[tok:toml] no filter matched — passthrough"),
        }
    }
    result
}
