...
3. **Degradation warnings**: print `[TOK:DEGRADED]` in verbose mode, `[TOK:PASSTHROUGH]` on full fallback
...
```
[TOK:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[TOK:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
