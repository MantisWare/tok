...
/// Run the `tok rewrite` command.
///
/// Prints the TOK-rewritten command to stdout and exits with a code that tells
/// the caller how to handle permissions:
///
/// | Exit | Stdout   | Meaning                                                      |
/// |------|----------|--------------------------------------------------------------|
/// | 0    | rewritten| Rewrite allowed — hook may auto-allow the rewritten command. |
/// | 1    | (none)   | No TOK equivalent — hook passes through unchanged.           |
...
pub fn run(cmd: &str) -> anyhow::Result<()> {
    ...
    // SECURITY: check deny/ask BEFORE rewrite so non-TOK commands are also covered.
...
mod tests {
    ...
    #[test]
    fn test_run_already_tok_returns_some() {
        assert_eq!(
            registry::rewrite_command("tok git status", &[]),
