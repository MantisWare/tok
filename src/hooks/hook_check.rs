//! Detects whether TOK hooks are installed and warns if they are outdated.
...
use crate::core::constants::TOK_DATA_DIR;
use std::path::PathBuf;

const CURRENT_HOOK_VERSION: u8 = 3;
const WARN_INTERVAL_SECS: u64 = 24 * 3600;

/// Hook status for diagnostics and `tok gain`.
...
fn check_and_warn() -> Option<()> {
    let warning = match status() {
        ...
        HookStatus::Missing => {
            "[tok] /!\\ No hook installed — run `tok init -g` for automatic token savings"
        }
        HookStatus::Outdated => "[tok] /!\\ Hook outdated — run `tok init -g` to update",
    };
...
pub fn parse_hook_version(content: &str) -> u8 {
    ...
    for line in content.lines().take(5) {
        if let Some(rest) = line.strip_prefix("# tok-hook-version:") {
...
fn warn_marker_path() -> Option<PathBuf> {
    let data_dir = dirs::data_local_dir()?.join(TOK_DATA_DIR);
