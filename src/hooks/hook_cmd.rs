...
enum HookFormat {
    ...
    /// Non-bash tool, already uses tok, or unknown format — pass through silently.
    PassThrough,
}
...
pub fn run_copilot() -> Result<()> {
    ...
    let v: Value = match serde_json::from_str(input) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[tok hook] Failed to parse JSON input: {e}");
            return Ok(());
        }
    };
...
/// Reads JSON from stdin, rewrites shell commands to tok equivalents,
...
pub fn run_gemini() -> Result<()> {
    ...
    if check_command(cmd) == PermissionVerdict::Deny {
        println!(r#"{{"decision":"deny","reason":"Blocked by TOK permission rule"}}"#);
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn test_get_rewritten_already_tok() {
        assert!(get_rewritten("tok git status").is_none());
    }
    ...
    fn test_gemini_hook_uses_rewrite_command() {
        ...
        // Already tok → returned as-is (idempotent)
        assert_eq!(
    ...
    fn test_gemini_hook_env_prefix_preserved() {
        assert_eq!(
            ...
            Some("RUST_LOG=debug tok cargo test".into())
        );
    }
}
