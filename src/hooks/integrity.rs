...
//!
//! Reference: SA-2025-TOK-001 (Finding F-01)
...
const HASH_FILENAME: &str = ".tok-hook.sha256";
...
pub enum IntegrityStatus {
    ...
    Verified,
    /// Hash mismatch — hook has been modified outside of `tok init`
    Tampered { expected: String, actual: String },
    /// Hook exists but no stored hash (installed before integrity checks)
    NoBaseline,
    /// Neither hook nor hash file exist (TOK not installed)
    NotInstalled,
...
/// Resolve the default hook path (~/.claude/hooks/tok-rewrite.sh)
...
/// Run integrity check and print results (for `tok verify` subcommand)
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  The hook file has been modified outside of `tok init`.");
            eprintln!("  This could indicate tampering or a manual edit.");
            eprintln!();
            eprintln!("  To restore: tok init -g --auto-patch");
        ...
        IntegrityStatus::NoBaseline => {
            ...
            println!("      Run `tok init -g` to establish baseline.");
        }
        IntegrityStatus::NotInstalled => {
            println!("SKIP  TOK hook not installed");
            println!("      Run `tok init -g` to install.");
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("WARN  hash file exists but hook is missing");
            eprintln!("      Run `tok init -g` to reinstall.");
        }
    }
...
/// re-run `tok init -g --auto-patch` to re-establish the baseline.
pub fn runtime_check() -> Result<()> {
    match verify_hook()? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            eprintln!("tok: hook integrity check FAILED");
            eprintln!(
                "  Expected hash: {}...",
            ...
            eprintln!();
            eprintln!("  The hook at ~/.claude/hooks/tok-rewrite.sh has been modified.");
            eprintln!("  This may indicate tampering. TOK will not execute.");
            eprintln!();
            eprintln!("  To restore:  tok init -g --auto-patch");
            eprintln!("  To inspect:  tok verify");
            std::process::exit(1);
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("tok: warning: hash file exists but hook is missing");
            eprintln!("  Run `tok init -g` to reinstall.");
...
mod tests {
    ...
    fn test_store_and_verify_ok() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("tok-rewrite.sh");
    ...
    fn test_verify_orphaned_hash() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("tok-rewrite.sh");
        let hash_file = temp.path().join(".tok-hook.sha256");

        // Create hash but no hook
        fs::write(
            &hash_file,
            "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2  tok-rewrite.sh\n",
        )
        .unwrap();
    ...
    fn test_store_hash_creates_sha256sum_format() {
        ...
        assert!(content.ends_with("  tok-rewrite.sh\n"));
        let parts: Vec<&str> = content.trim().splitn(2, "  ").collect();
        assert_eq!(parts.len(), 2);
        assert_eq!(parts[0].len(), 64);
        assert_eq!(parts[1], "tok-rewrite.sh");
    }
    ...
    fn test_invalid_hash_file_rejected() {
        ...
        fs::write(&hook, "test").unwrap();
        fs::write(&hash_file, "not-a-valid-hash  tok-rewrite.sh\n").unwrap();
