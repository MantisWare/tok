...
//!
//! `.tok/filters.toml` is loaded from CWD with highest priority. An attacker
...
//! - `tok trust` stores the SHA-256 hash after user review
//! - Content changes invalidate trust (re-review required)
//! - `TOK_TRUST_PROJECT_FILTERS=1` overrides for CI pipelines

use super::integrity;
use crate::core::constants::{TOK_DATA_DIR, TRUSTED_FILTERS_JSON};
...
fn store_path() -> Result<PathBuf> {
    ...
    Ok(data_dir.join(TOK_DATA_DIR).join(TRUSTED_FILTERS_JSON))
}
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
    ...
    if std::env::var("TOK_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
        ...
        eprintln!(
            "[tok] WARNING: TOK_TRUST_PROJECT_FILTERS=1 ignored (CI environment not detected)"
        );
    }
    ...
    let store = match read_store() {
        ...
        Err(e) => {
            eprintln!(
                "[tok] WARNING: trust store unreadable ({}), treating all filters as untrusted",
                e
            );
            TrustStore::default()
        }
...
/// Run `tok trust` — review and trust project-local filters.
pub fn run_trust(list: bool) -> Result<()> {
    ...
    let content_bytes = std::fs::read(filter_path).context("Failed to read .tok/filters.toml")?;
    let content = String::from_utf8_lossy(&content_bytes);

    println!("=== .tok/filters.toml ===");
    println!("{}", content);
    println!("=========================");
    println!();
    ...
    println!();
    println!(
        "Trusted .tok/filters.toml (sha256:{})",
...
/// Run `tok untrust` — revoke trust for project-local filters.
pub fn run_untrust() -> Result<()> {
    ...
    if removed {
        println!("Trust revoked for .tok/filters.toml");
...
mod tests {
    ...
    fn test_env_override_with_ci() {
        ...
        #[allow(deprecated)]
        std::env::set_var("TOK_TRUST_PROJECT_FILTERS", "1");
        #[allow(deprecated)]
        std::env::set_var("CI", "true");
        let status = check_trust(&filter).unwrap();
        #[allow(deprecated)]
        std::env::remove_var("TOK_TRUST_PROJECT_FILTERS");
        #[allow(deprecated)]
