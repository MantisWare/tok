...
pub const RULES: &[TokRule] = &[
    TokRule {
        pattern: r"^git\s+(?:-[Cc]\s+\S+\s+)*(status|log|diff|show|add|commit|push|pull|branch|fetch|stash|worktree|blame|rev-parse|describe|tag|ls-files|checkout|merge|rebase|remote|config|cherry-pick|shortlog|reflog|grep)",
        tok_cmd: "tok git",
    ...
    TokRule {
        pattern: r"^pnpm\s+(list|ls|outdated|install|why|run|exec|add|remove)",
        tok_cmd: "tok pnpm",
    ...
    TokRule {
        pattern: r"^npm\s+(run|exec|install|ci|test|ls)",
        tok_cmd: "tok npm",
    ...
    TokRule {
        ...
        tok_cmd: "tok spring-boot",
        rewrite_prefixes: &["mvn spring-boot:run", "gradle bootRun"],
        category: "Build",
        savings_pct: 70.0,
        subcmd_savings: &[],
        subcmd_status: &[],
