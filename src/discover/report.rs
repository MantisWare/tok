//! Data types for reporting which commands TOK can and cannot optimize.

use crate::hooks::constants::{HOOKS_SUBDIR, REWRITE_HOOK_FILE};
use serde::Serialize;

/// TOK support status for a command.
...
pub enum TokStatus {
    ...
    Passthrough,
    /// TOK doesn't handle this command at all
    NotSupported,
}
...
impl TokStatus {
    pub fn as_str(&self) -> &'static str {
        match self {
            TokStatus::Existing => "existing",
            TokStatus::Passthrough => "passthrough",
            TokStatus::NotSupported => "not-supported",
        }
    }
}

/// A supported command that TOK already handles.
#[derive(Debug, Serialize)]
pub struct SupportedEntry {
    pub command: String,
    pub count: usize,
    pub tok_equivalent: &'static str,
    pub category: &'static str,
    pub estimated_savings_tokens: usize,
    pub estimated_savings_pct: f64,
    pub tok_status: TokStatus,
}

/// An unsupported command not yet handled by TOK.
#[derive(Debug, Serialize)]
...
pub struct DiscoverReport {
    ...
    pub already_tok: usize,
    pub since_days: u64,
    pub supported: Vec<SupportedEntry>,
    pub unsupported: Vec<UnsupportedEntry>,
    pub parse_errors: usize,
    pub tok_disabled_count: usize,
    pub tok_disabled_examples: Vec<String>,
}
...
pub fn format_text(report: &DiscoverReport, limit: usize, verbose: bool) -> String {
    ...
    out.push_str("TOK Discover -- Savings Opportunities\n");
    ...
    out.push_str(&format!(
        "Already using TOK: {} commands ({}%)\n",
        report.already_tok,
        if report.total_commands > 0 {
            report.already_tok * 100 / report.total_commands
        } else {
            0
        }
    ));
    ...
    if report.supported.is_empty() && report.unsupported.is_empty() {
        out.push_str("\nNo missed savings found. TOK usage looks good!\n");
        return out;
    }

    // Missed savings
    if !report.supported.is_empty() {
        out.push_str("\nMISSED SAVINGS -- Commands TOK already handles\n");
        out.push_str(&"-".repeat(72));
        out.push('\n');
        out.push_str(&format!(
            "{:<24} {:>5}    {:<18} {:<13} {:>12}\n",
            "Command", "Count", "TOK Equivalent", "Status", "Est. Savings"
        ));
        ...
        for entry in report.supported.iter().take(limit) {
            out.push_str(&format!(
                ...
                entry.count,
                entry.tok_equivalent,
                entry.tok_status.as_str(),
    ...
    if report.tok_disabled_count > 0 {
        out.push_str(&format!(
            "\nTOK_DISABLED BYPASS -- {} commands ran without filtering\n",
            report.tok_disabled_count
        ));
        out.push_str(&"-".repeat(72));
        out.push('\n');
        out.push_str("These commands used TOK_DISABLED=1 unnecessarily:\n");
        if !report.tok_disabled_examples.is_empty() {
            out.push_str(&format!("  {}\n", report.tok_disabled_examples.join(", ")));
        }
        out.push_str("-> Remove TOK_DISABLED=1 to recover token savings\n");
    }
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        if cursor_hook.exists() {
            out.push_str("\nNote: Cursor sessions are tracked via `tok gain` (discover scans Claude Code only)\n");
        }
    }
