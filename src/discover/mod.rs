//! Scans AI coding sessions to find commands that could benefit from TOK filtering.
...
use registry::{
    category_avg_tokens, classify_command, has_tok_disabled_prefix, split_command_chain,
...
pub fn run(
    ...
    let mut already_tok: usize = 0;
    let mut parse_errors: usize = 0;
    let mut tok_disabled_count: usize = 0;
    let mut tok_disabled_cmds: HashMap<String, usize> = HashMap::new();
    ...
    for session_path in &sessions {
        ...
        for ext_cmd in &extracted {
            ...
            for part in parts {
                ...
                // Detect TOK_DISABLED= bypass before classification
                if has_tok_disabled_prefix(part) {
                    let actual_cmd = strip_disabled_prefix(part);
                    // Only count if the underlying command is one TOK supports
                    match classify_command(actual_cmd) {
                        Classification::Supported { .. } => {
                            tok_disabled_count += 1;
                            let display = truncate_command(actual_cmd);
                            *tok_disabled_cmds.entry(display).or_insert(0) += 1;
                        }
                        _ => {
                            // TOK_DISABLED on unsupported/ignored command — not interesting
                        }
                    }
                    continue;
                }
                ...
                match classify_command(part) {
                    Classification::Supported {
                        ...
                        let bucket = supported_map.entry(tok_equivalent).or_insert_with(|| {
                            SupportedBucket {
                                tok_equivalent,
                                category,
                                count: 0,
                    ...
                    Classification::Ignored => {
                        // Check if it starts with "tok "
                        if part.trim().starts_with("tok ") {
                            already_tok += 1;
                        }
                        // Otherwise just skip
                    }
                ...
                .map(|(name, _)| {
                    ...
                    if let Some(colon_pos) = name.rfind(':') {
                        ...
                        let status = match status_str {
                            "Passthrough" => report::TokStatus::Passthrough,
                            "NotSupported" => report::TokStatus::NotSupported,
                            _ => report::TokStatus::Existing,
                        };
                        (cmd, status)
                    } else {
                        (name, report::TokStatus::Existing)
                    }
                })
                .unwrap_or_else(|| (String::new(), report::TokStatus::Existing));

            SupportedEntry {
                command: command_with_status,
                count: bucket.count,
                tok_equivalent: bucket.tok_equivalent,
    ...
    // Build TOK_DISABLED examples sorted by frequency (top 5)
    let tok_disabled_examples: Vec<String> = {
        let mut sorted: Vec<_> = tok_disabled_cmds.into_iter().collect();
