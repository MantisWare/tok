...
pub fn run(
    ...
    if reset {
        return run_reset(&tracker, project_scope.as_deref(), format);
    }
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        let title = if project_scope.is_some() {
            "TOK Token Savings (Project Scope)"
        } else {
            "TOK Token Savings (Global Scope)"
        };
        ...
        match hook_check::status() {
            hook_check::HookStatus::Missing => {
                eprintln!(
                    "{}",
                    "[warn] No hook installed — run `tok init -g` for automatic token savings"
                        .yellow()
                );
                eprintln!();
            }
            hook_check::HookStatus::Outdated => {
                eprintln!(
                    "{}",
                    "[warn] Hook outdated — run `tok init -g` to update".yellow()
                );
                eprintln!();
            }
        ...
        // Lightweight TOK_DISABLED bypass check (best-effort, silent on failure)
        if let Some(warning) = check_tok_disabled_bypass() {
        ...
        if history {
            ...
            if !recent.is_empty() {
                ...
                for rec in recent {
                    ...
                    let cmd_short = if rec.tok_cmd.len() > 25 {
                        format!("{}...", &rec.tok_cmd[..22])
                    } else {
                        rec.tok_cmd.clone()
                    };
...
fn run_reset(tracker: &Tracker, project_scope: Option<&str>, format: &str) -> Result<()> {
    ...
    if format == "json" {
        ...
        struct ResetJson<'a> {
            ...
            parse_failures_removed: usize,
        }
        let payload = ResetJson {
            ...
            parse_failures_removed: result.parse_failures_removed,
        };
        println!(
            "{}",
            serde_json::to_string_pretty(&payload).context("Failed to serialize reset result")?
        );
        return Ok(());
    }
    ...
    match project_scope {
        Some(_) => {
            println!(
                "Cleared {} command record(s) for the current project (parse-failure log unchanged).",
                result.commands_removed
            );
        }
        None => {
            println!(
                "Cleared {} command record(s) and {} parse failure record(s).",
                result.commands_removed, result.parse_failures_removed
            );
        }
    }
    Ok(())
...
/// Lightweight scan of recent Claude Code sessions for TOK_DISABLED= overuse.
...
fn check_tok_disabled_bypass() -> Option<String> {
    use crate::discover::provider::{ClaudeProvider, SessionProvider};
    use crate::discover::registry::has_tok_disabled_prefix;
    ...
    for session_path in &sessions {
        ...
        for ext_cmd in &extracted {
            total_bash += 1;
            if has_tok_disabled_prefix(&ext_cmd.command) {
                bypassed += 1;
            }
        }
    }
    ...
    if pct > 10.0 {
        Some(format!(
            "[warn] {} commands ({:.0}%) used TOK_DISABLED=1 unnecessarily — run `tok discover` for details",
            bypassed, pct
        ))
    } else {
        None
...
fn show_failures(tracker: &Tracker) -> Result<()> {
    ...
    println!("{}", styled("TOK Parse Failures", true));
    println!("{}", "═".repeat(60));
    println!();
