...
//!
//! Combines ccusage (tokens spent) with tok tracking (tokens saved) to provide
...
pub struct PeriodEconomics {
    ...
    // tok metrics
    pub tok_commands: Option<usize>,
    pub tok_saved_tokens: Option<usize>,
    pub tok_savings_pct: Option<f64>,
...
impl PeriodEconomics {
    fn new(label: &str) -> Self {
        Self {
            ...
            tok_commands: None,
            tok_saved_tokens: None,
            tok_savings_pct: None,
    ...
    fn set_tok_from_day(&mut self, stats: &DayStats) {
        self.tok_commands = Some(stats.commands);
        self.tok_saved_tokens = Some(stats.saved_tokens);
        self.tok_savings_pct = Some(stats.savings_pct);
    }

    fn set_tok_from_week(&mut self, stats: &WeekStats) {
        self.tok_commands = Some(stats.commands);
        self.tok_saved_tokens = Some(stats.saved_tokens);
        self.tok_savings_pct = Some(stats.savings_pct);
    }

    fn set_tok_from_month(&mut self, stats: &MonthStats) {
        self.tok_commands = Some(stats.commands);
        self.tok_saved_tokens = Some(stats.saved_tokens);
        self.tok_savings_pct = Some(if stats.input_tokens + stats.output_tokens > 0 {
    ...
    fn compute_weighted_metrics(&mut self) {
        ...
        if let (Some(cost), Some(saved)) = (self.cc_cost, self.tok_saved_tokens) {
...
struct Totals {
    ...
    tok_commands: usize,
    tok_saved_tokens: usize,
    tok_avg_savings_pct: f64,
...
fn merge_daily(cc: Option<Vec<CcusagePeriod>>, tok: Vec<DayStats>) -> Vec<PeriodEconomics> {
...
fn merge_weekly(cc: Option<Vec<CcusagePeriod>>, tok: Vec<WeekStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
    ...
    // Merge tok data (week_start = legacy Saturday "2026-01-18")
...
fn merge_monthly(cc: Option<Vec<CcusagePeriod>>, tok: Vec<MonthStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
...
/// Convert Saturday week_start (legacy tok) to ISO Monday
...
fn convert_saturday_to_monday(saturday: &str) -> Option<String> {
    ...
    // tok uses Saturday as week start, ISO uses Monday
...
fn compute_totals(periods: &[PeriodEconomics]) -> Totals {
    ...
    for p in periods {
        ...
        if let Some(cmds) = p.tok_commands {
            totals.tok_commands += cmds;
        }
        if let Some(saved) = p.tok_saved_tokens {
            totals.tok_saved_tokens += saved;
        }
        if let Some(pct) = p.tok_savings_pct {
            pct_sum += pct;
            pct_count += 1;
        }
    }
    ...
    if pct_count > 0 {
        totals.tok_avg_savings_pct = pct_sum / pct_count as f64;
    }
    ...
    if weighted_units > 0.0 {
        ...
        totals.savings_weighted = Some(totals.tok_saved_tokens as f64 * input_cpt);
    }
    ...
    if totals.cc_total_tokens > 0 {
        ...
        totals.savings_blended = Some(totals.tok_saved_tokens as f64 * totals.blended_cpt.unwrap());
    }
    if totals.cc_active_tokens > 0 {
        totals.active_cpt = Some(totals.cc_cost / totals.cc_active_tokens as f64);
        totals.savings_active = Some(totals.tok_saved_tokens as f64 * totals.active_cpt.unwrap());
    }
...
fn display_summary(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    let tok_monthly = tracker
        .get_by_month()
        .context("Failed to load monthly token savings from database")?;
    let periods = merge_monthly(cc_monthly, tok_monthly);

    if periods.is_empty() {
        println!("No data available. Run some tok commands to start tracking.");
        return Ok(());
    }
    ...
    println!("  TOK commands:                 {}", totals.tok_commands);
    println!(
        "  Tokens saved:                 {}",
        format_tokens(totals.tok_saved_tokens)
    );
    println!();
    ...
    println!("  TOK compresses CLI outputs before they enter Claude's context.");
...
fn display_daily(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    let tok_daily = tracker
        .get_all_days()
        .context("Failed to load daily token savings from database")?;
    let periods = merge_daily(cc_daily, tok_daily);
...
fn display_weekly(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    let tok_weekly = tracker
        .get_by_week()
        .context("Failed to load weekly token savings from database")?;
    let periods = merge_weekly(cc_weekly, tok_weekly);
...
fn print_period_table(periods: &[PeriodEconomics], verbose: u8) {
    ...
    if verbose > 0 {
        ...
        println!(
            ...
            "Period", "Spent", "Saved", "Savings", "Active$", "Blended$", "TOK Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<10} {:-<12} {:-<12}",
            "", "", "", "", "", "", ""
        ...
        println!(
            "{:<12} {:>10} {:>10} {:>10} {:>12}",
            "Period", "Spent", "Saved", "Savings", "TOK Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<12}",
            "", "", "", "", ""
...
fn export_json(
    ...
    if all || daily {
        ...
        export.daily = Some(merge_daily(cc, tok));
    }
    ...
    if all || weekly {
        ...
        export.weekly = Some(merge_weekly(cc, tok));
    }
    ...
    if all || monthly {
        ...
        let periods = merge_monthly(cc, tok);
...
fn export_csv(
    ...
    println!("period,spent,input_tokens,output_tokens,cache_create,cache_read,active_tokens,total_tokens,saved_tokens,weighted_savings,active_savings,blended_savings,tok_commands");
    ...
    if all || daily {
        ...
        let tok = tracker
            .get_all_days()
            .context("Failed to load daily token savings for JSON export")?;
        let periods = merge_daily(cc, tok);
        for p in periods {
            print_csv_row(&p);
        }
    }
    ...
    if all || weekly {
        ...
        let tok = tracker
            .get_by_week()
            .context("Failed to load weekly token savings for export")?;
        let periods = merge_weekly(cc, tok);
        for p in periods {
            print_csv_row(&p);
        }
    }
...
fn print_csv_row(p: &PeriodEconomics) {
    ...
    let blended_savings = p
        ...
        .map(|s| format!("{:.4}", s))
        .unwrap_or_default();
    let cmds = p.tok_commands.map(|c| c.to_string()).unwrap_or_default();
...
mod tests {
    ...
    fn test_merge_monthly_both_present() {
        ...
        let merged = merge_monthly(Some(cc), tok);
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].label, "2026-01");
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert_eq!(merged[0].tok_commands, Some(10));
    }
    ...
    fn test_merge_monthly_only_ccusage() {
        ...
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert!(merged[0].tok_commands.is_none());
    }

    #[test]
    fn test_merge_monthly_only_tok() {
        let tok = vec![MonthStats {
            month: "2026-01".to_string(),
            commands: 10,
            input_tokens: 800,
        ...
        let merged = merge_monthly(None, tok);
        assert_eq!(merged.len(), 1);
    ...
    fn test_compute_weighted_input_cpt() {
        ...
        p.tok_saved_tokens = Some(10_000);
    ...
    fn test_compute_totals() {
        let periods = vec![
            PeriodEconomics {
                ...
                tok_commands: Some(5),
                tok_saved_tokens: Some(2000),
                tok_savings_pct: Some(50.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
            ...
            PeriodEconomics {
                ...
                tok_commands: Some(10),
                tok_saved_tokens: Some(3000),
                tok_savings_pct: Some(60.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
        ...
        assert_eq!(totals.tok_commands, 15);
        assert_eq!(totals.tok_saved_tokens, 5000);
        assert_eq!(totals.tok_avg_savings_pct, 55.0);
