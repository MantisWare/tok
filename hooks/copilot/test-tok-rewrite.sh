...
# Feeds mock preToolUse JSON through `tok hook` and verifies allow/deny decisions.
#
...
test_deny() {
  ...
  local output
  output=$(copilot_bash_input "$input_cmd" | "$TOK" hook 2>/dev/null) || true
...
test_vscode_rewrite() {
  ...
  local output
  output=$(vscode_bash_input "$input_cmd" | "$TOK" hook 2>/dev/null) || true
...
test_allow() {
  ...
  local output
  output=$(echo "$input" | "$TOK" hook 2>/dev/null) || true
...
echo "============================================"
echo "  TOK Hook Test Suite (tok hook)"
echo "============================================"
echo ""
...
TOTAL=$((TOTAL + 1))
raw_output=$(copilot_bash_input "git status" | "$TOK" hook 2>/dev/null)
...
TOTAL=$((TOTAL + 1))
vscode_output=$(vscode_bash_input "git status" | "$TOK" hook 2>/dev/null)
