---
model: sonnet
description: TOK Codebase Health Audit — 7 catégories scorées 0-10
argument-hint: "[--category <cat>] [--fix] [--json]"
allowed-tools: [Read, Grep, Glob, Bash, Write]
---

# Audit Codebase — Santé du Projet TOK
...
# Dépendances async (interdit dans TOK)
Grep "tokio\|async-std\|futures" Cargo.toml

# Taille binaire post-strip
ls -lh target/release/tok 2>/dev/null || echo "Build needed"
```
...
**Objectif** : Architecture TOK respectée, conventions Rust appliquées.
...
which hyperfine && hyperfine 'tok git status' --warmup 3 2>&1 | grep "Time"

# Mémoire binaire
ls -lh target/release/tok 2>/dev/null
