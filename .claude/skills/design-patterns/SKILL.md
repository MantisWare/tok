...
description: Rust design patterns for TOK. Newtype, Builder, RAII, Trait Objects, State Machine. Applied to CLI filter modules. Use when designing new modules or refactoring existing ones.
triggers:
  - "design pattern"
  - "how to structure"
  - "best pattern for"
...
allowed-tools:
  ...
  - Glob
effort: medium
tags: [rust, design-patterns, architecture, newtype, builder, tok]
---

# TOK Rust Design Patterns

Patterns that apply to TOK's filter module architecture. Focused on CLI tool patterns, not web/service patterns.
...
```rust
// Practical TOK example: command name validation
...
```rust
// TOK example: pytest output parsing
...
// TOK currently uses match-based dispatch in main.rs (simpler, no dynamic dispatch overhead)
// Trait objects are useful if filter registry becomes dynamic (e.g., TOML-loaded plugins)
```

Note: TOK's current `match` dispatch in `main.rs` is intentional — static dispatch, zero overhead. Only move to trait objects if the match arm count exceeds ~20 commands.
...
```rust
// TOK tee.rs — RAII for temp output files
pub struct TeeFile {
    path: PathBuf,
}
...
Use when: you need to add methods to types you don't own (like `&str` for TOK-specific parsing).
...
// ✅ Synchronous — TOK is single-threaded by design
