...
Systematic release workflow for TOK: build verification, version bump, changelog update, git tag, and push to trigger CI/CD.
...
/usr/bin/time -l target/release/tok git status
...
### Added
- `tok pytest` command for Python test filtering (90% token reduction)
...
Added:
- tok pytest command (90% token reduction)
...
target/release/tok --version | grep "$NEW_VERSION"
...
**Symptom**: `tok --version` shows old version after bump
