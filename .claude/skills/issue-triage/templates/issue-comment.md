...
- {Specific missing info 1 — e.g., "TOK version (`tok --version` output)"}
...
Use when: issue requests something that doesn't align with TOK's design goals (e.g., adding async runtime, platform-specific features outside scope, changing core behavior).

```markdown
## Closing: Out of Scope

After review, this request falls outside TOK's current design goals.

### Rationale

{1-2 sentences explaining why — be specific. Reference design constraints if relevant, e.g., "TOK is intentionally single-threaded with zero async dependencies to maintain <10ms startup time."}

### Alternatives

{If applicable: what the user can do instead. E.g., "For this use case, `tok proxy <cmd>` gives you raw output while still tracking usage metrics."}
...
**TOK-specific context to include when relevant** :
- Mention `tok --version` as the first diagnostic step for bug reports
