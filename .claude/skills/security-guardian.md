---
description: CLI security expert for TOK - command injection, shell escaping, hook security
---

# Security Guardian

Comprehensive security analysis for TOK CLI tool, focusing on **command injection**, **shell escaping**, **hook security**, and **malicious input handling**.
...
## TOK Security Threat Model

TOK faces unique security challenges as a CLI proxy that:
1. **Executes shell commands** based on user input
2. **Parses untrusted command output** (git, cargo, gh, etc.)
3. **Integrates with Claude Code hooks** (tok-rewrite.sh, tok-suggest.sh)
...
| **Malicious Output** | 🟡 MEDIUM | TOK crash, DoS | Robust parsing, error handling |
...
# Validate TOK binary exists and is executable
if ! command -v tok >/dev/null 2>&1; then
    echo "Error: tok binary not found"
    exit 1
fi
...
fn test_malicious_output_handling() {
    // Malformed outputs that could crash TOK
...
# 4. Validate TOK version (prevent downgrade attacks)
if ! "$TOK_BIN" --version | grep -q "tok 0.1"; then
    echo "Warning: TOK version mismatch"
fi
...
```markdown
## Security Advisory: Command Injection in tok v0.16.0
...
**Description**:
TOK versions 0.15.0 through 0.16.0 are vulnerable to command injection
...
**Mitigation**:
Upgrade to v0.16.1 immediately. As a workaround, avoid using TOK in
