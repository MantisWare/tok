...
Systematic performance analysis and optimization for TOK CLI tool, focusing on **startup time (<10ms)**, **memory usage (<5MB)**, and **token savings (60-90%)**.
...
## TOK Performance Targets

| Metric | Target | Verification Method | Failure Threshold |
|--------|--------|---------------------|-------------------|
| **Startup time** | <10ms | `hyperfine 'tok <cmd>'` | >15ms = blocker |
| **Memory usage** | <5MB resident | `/usr/bin/time -l tok <cmd>` (macOS) | >7MB = blocker |
| **Token savings** | 60-90% | Tests with `count_tokens()` | <60% = blocker |
| **Binary size** | <5MB stripped | `ls -lh target/release/tok` | >8MB = investigate |
...
hyperfine 'tok git status' --warmup 3 --export-json /tmp/baseline_startup.json

# Memory usage baseline (macOS)
/usr/bin/time -l tok git status 2>&1 | grep "maximum resident set size" > /tmp/baseline_memory.txt

# Memory usage baseline (Linux)
/usr/bin/time -v tok git status 2>&1 | grep "Maximum resident set size" > /tmp/baseline_memory.txt

# Binary size baseline
ls -lh target/release/tok | tee /tmp/baseline_binary_size.txt
```
...
hyperfine 'target/release/tok git status' --warmup 3 --export-json /tmp/after_startup.json

# Measure memory usage
/usr/bin/time -l target/release/tok git status 2>&1 | grep "maximum resident set size" > /tmp/after_memory.txt

# Check binary size
ls -lh target/release/tok | tee /tmp/after_binary_size.txt
```
...
hyperfine 'tok git status' 'target/release/tok git status' --warmup 3

# Example output:
#   Benchmark 1: tok git status
#     Time (mean ± σ):       6.2 ms ±   0.3 ms    [User: 4.1 ms, System: 1.8 ms]
#   Benchmark 2: target/release/tok git status
#     Time (mean ± σ):       7.8 ms ±   0.4 ms    [User: 5.2 ms, System: 2.1 ms]
#
#   Summary
#     'tok git status' ran 1.26 times faster than 'target/release/tok git status'
...
cargo flamegraph -- target/release/tok git status
...
cargo flamegraph -- target/release/tok git log -10
...
valgrind --tool=dhat target/release/tok git status
```
...
```bash
# strace on Linux
strace -c target/release/tok git status 2>&1 | grep -E "open|read"

# dtrace on macOS (requires SIP disabled)
sudo dtrace -n 'syscall::open*:entry { @[execname] = count(); }' &
target/release/tok git status
sudo pkill dtrace
```
...
fn main() {
    let config = load_config().unwrap(); // Reads ~/.config/tok/config.toml
    // ...
}
...
### Binary Size
- [ ] Check `ls -lh target/release/tok`
...
CURRENT_TIME=$(hyperfine 'tok git status' --warmup 3 --export-json /tmp/perf.json 2>&1 | grep "Time (mean" | awk '{print $4}')
...
# Check binary size
BINARY_SIZE=$(ls -l target/release/tok | awk '{print $5}')
...
- name: Performance Regression Check
  run: |
    ...
    # Benchmark startup time
    hyperfine 'target/release/tok git status' --warmup 3 --max-runs 10
...
|------|---------|---------|
| **hyperfine** | Benchmark startup time | `hyperfine 'tok <cmd>' --warmup 3` |
| **time** | Memory usage (macOS) | `/usr/bin/time -l tok <cmd>` |
| **time** | Memory usage (Linux) | `/usr/bin/time -v tok <cmd>` |
| **flamegraph** | CPU profiling | `cargo flamegraph -- tok <cmd>` |
...
| **strace** | System call tracing (Linux) | `strace -c target/release/tok <cmd>` |
