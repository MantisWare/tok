...
TOK is a **zero-overhead CLI proxy**. Every architectural decision must be evaluated against:
...
```
~/.config/tok/filters/    ← User-global filters
<project>/.tok/filters/   ← Project-local filters (shadow warning)
```

## Architectural Patterns (TOK Idioms)
...
**Will not:**
- Implement the full filter logic (→ rust-tok agent)
