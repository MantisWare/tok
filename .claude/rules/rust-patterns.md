# Rust Patterns — TOK Development Rules

TOK-specific Rust idioms and constraints. Applied to all code in this repository.

## Non-Negotiable TOK Rules
...
Note: `lazy_static!` with `.unwrap()` for initialization is the **established TOK pattern** — it's acceptable because a bad regex literal is a programming error caught at first use.
...
// ❌ Silent swallow — catastrophic in TOK (user gets no output)
match result {
    Ok(output) => process(output),
    Err(_) => {}
}
