# Search Strategy — TOK Codebase Navigation

Efficient search patterns for TOK's Rust codebase.
...
│   ├── config.rs              ← ~/.config/tok/config.toml
...
3. `TOK_DB_PATH` env var overrides config

### Configuration issues

1. `src/core/config.rs` → `TokConfig` struct
2. `src/hooks/init.rs` → `tok init` command
3. Config file: `~/.config/tok/config.toml`
4. Filter files: `~/.config/tok/filters/` (global) or `.tok/filters/` (project)

## TOML Filter DSL Navigation

```
Glob pattern=".tok/filters/*.toml"         # Project-local filters
