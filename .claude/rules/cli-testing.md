...
Comprehensive testing rules for TOK CLI tool development.

## Snapshot Testing (🔴 Critical)

**Priority**: 🔴 **Triggers**: All filter changes, output format modifications

Use `insta` crate for output validation. This is the **primary testing strategy** for TOK filters.
...
TOK targets <10ms startup time and <5MB memory usage.
...
# Benchmark TOK vs raw command
hyperfine 'tok git status' 'git status' --warmup 3

# Should show TOK startup <10ms
# Example output:
#   tok git status    6.2 ms ±  0.3 ms
...
```bash
# macOS
/usr/bin/time -l tok git status
# Look for "maximum resident set size" - should be <5MB

# Linux
/usr/bin/time -v tok git status
...
**Before changes**:
```bash
hyperfine 'tok git log -10' --warmup 3 > /tmp/before.txt
```

**After changes**:
```bash
cargo build --release
hyperfine 'target/release/tok git log -10' --warmup 3 > /tmp/after.txt
```
...
|--------|--------|--------------|
| Startup time | <10ms | `hyperfine 'tok <cmd>'` |
| Memory usage | <5MB | `time -l tok <cmd>` |
| Binary size | <5MB | `ls -lh target/release/tok` |
...
hyperfine 'tok cmd' --warmup 3 > /tmp/before.txt
# Make changes
cargo build --release
hyperfine 'target/release/tok cmd' --warmup 3 > /tmp/after.txt
diff /tmp/before.txt /tmp/after.txt
```
