#!/usr/bin/env bash
# Install TOK from a local release build (builds from source, no network download).

set -euo pipefail

INSTALL_DIR="${1:-$HOME/.cargo/bin}"
INSTALL_PATH="${INSTALL_DIR}/tok"
BINARY_PATH="./target/release/tok"
